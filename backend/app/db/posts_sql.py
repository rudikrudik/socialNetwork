...
def get_post_limit_and_offset(user_id: int, posts_limit: int, offset: int) -> list:
    list_posts = []
    id_posts = raw_query(f"SELECT (id) FROM user_posts WHERE user_id IN"
    ...
    print("Id post:", id_posts)

    if id_posts:
        for id_post in id_posts:
            list_posts.extend(get_user_post_by_id(id_post))
